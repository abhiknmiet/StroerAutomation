...
import com.sogeti.util.BrowserFactory;


/**
 * This class defines all the elements for a specific page. Every Class which
 * contains page objects should extend "SogetiUtility" class This gives access to
 ...
 *
 */
public class SogetiUtility extends BrowserFactory {
	...
	/**
	 * Method for click of Element By CSS selector
	 * 
	 * @throws InterruptedException
	 */
	public void clickInputElementByCssSelector(String value) {
		clickOnControl(By.cssSelector(value));
	}
	...
	/**
	 * Method for getting WebElement By CSS selector
	 */
	public WebElement getWebElementByCssSelector(String cssSelector) {
		
		WebElement webElement = getWebElement(By.cssSelector(cssSelector));
		return webElement;
	}
	...
	private boolean PerformControlAction(TestAutoControlInput SOGETIControlitem) {
		// Do not print value of Password control
		if (SOGETIControlitem.controlIdentifier.toString().contains("assword")) {
			System.out.println("[" + new SimpleDateFormat("yyyy.MM.dd HH.mm.ss").format(new Date())
					+ "] : Action to start for Control Type : " + SOGETIControlitem.controlType + " - Identifier : "
					+ SOGETIControlitem.controlIdentifier);
		} else {
			System.out.println("[" + new SimpleDateFormat("yyyy.MM.dd HH.mm.ss").format(new Date())
					+ "] : Action to start for Control Type : " + SOGETIControlitem.controlType + " - Identifier : "
					+ SOGETIControlitem.controlIdentifier + " - Value : " + SOGETIControlitem.valueToUse);
		}

		WebElement webElement = getWebElement(SOGETIControlitem.controlIdentifier);

		try {
			if (webElement != null) {
				switch (SOGETIControlitem.controlType) {
				case Textbox:
					fillInputValue(webElement, SOGETIControlitem.valueToUse);
					break;
				case Dropdown:
					selectDropDownItem(webElement, SOGETIControlitem.valueToUse, SOGETIControlitem.dropdownSelectBy);
					break;
				case SetFocus:
					setFocus(webElement);
					break;
				...
				System.out.println("[" + new SimpleDateFormat("yyyy.MM.dd HH.mm.ss").format(new Date())
						+ "] : Action completed for - Control Type : " + SOGETIControlitem.controlType
						+ " - Identifier : " + SOGETIControlitem.controlIdentifier);
				return true;
			} else {
				takeScreenShot();
				System.out.println("[" + new SimpleDateFormat("yyyy.MM.dd HH.mm.ss").format(new Date())
						+ "] : Could not find Webelement, so Action not completed for - Control Type : "
						+ SOGETIControlitem.controlType + " - Identifier : " + SOGETIControlitem.controlIdentifier);
				Reporter.addStepLog("[" + new SimpleDateFormat("yyyy.MM.dd HH.mm.ss").format(new Date())
						+ "] : Element Control Type : " + SOGETIControlitem.controlType + " - Identifier : "
						+ SOGETIControlitem.controlIdentifier + " not found");
